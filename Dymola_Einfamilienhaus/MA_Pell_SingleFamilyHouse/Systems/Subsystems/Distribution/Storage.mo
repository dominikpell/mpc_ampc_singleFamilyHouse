...
model Storage
  ...
  parameter Real T_start_layers[n] = fill(T_start, n) annotation(Dialog(tab = "Initialization"));
  parameter Real T_start_layers_HE[n] = fill(T_start, n) annotation(Dialog(tab = "Initialization"));
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalConductor heatTransfer[n](final G=
        if n == 1 then (array(G_middle + 2*lambda_ins/s_ins*A for k in 1:n))
         else cat(
          1,
          {G_top_bottom},
