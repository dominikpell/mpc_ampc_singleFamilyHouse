...
model Storage_TES
  ...
  Modelica.Fluid.Interfaces.FluidPort_a port_a_consumer_heating(redeclare
      final package Medium = Medium) annotation (Placement(transformation(
  ...
  Modelica.Thermal.HeatTransfer.Components.ThermalConductor heatTransfer_HE[n](
      each final G=k_HE*A_HE/n)
...
1,
{G_top_bottom},
array(G_middle for k in 2:n - 1),
{G_top_bottom}))                                                                                                                                      annotation(Placement(transformation(extent = {{-80, -10}, {-60, 10}})));
  ...
  Modelica.Fluid.Interfaces.FluidPort_b port_b_consumer_bot(redeclare final
      package Medium = Medium) annotation (Placement(transformation(extent={{-50,
...
equation
  ...
  connect(port_a_consumer_heating, layer[1].ports[1]) annotation (Line(points={{-26,-98},
          {-26,-34},{20,-34},{20,-2},{10,-2}},                      color={0,127,
          255}));
  connect(layer[n].ports[2], port_b_consumer_top) annotation (Line(points={{10,2},{
          16,2},{20,2},{20,40},{0,40},{0,92}},  color={0,127,255}));

  //Connect layers
  for k in 1:n - 1 loop
    connect(
  layer[k].ports[2], layer[k + 1].ports[1]);
  end for;
  //Connect Heat Transfer to the Outside
  for k in 1:n loop
    connect(
  heatTransfer[k].port_a, layer[k].heatPort);
    connect(
  heatTransfer[k].port_b, heatPort);
  end for;
  //Connect layers of Heat Exchanger
  connect(port_a_heatGenerator_heating, layer_HE[n].ports[2]) annotation (Line(
        points={{82,88},{82,36},{98,36},{98,2},{94,2},{94,2}},            color=
         {0,127,255}));
  connect(port_b_heatGenerator_heating, layer_HE[1].ports[1]) annotation (Line(
        points={{70,-98},{70,-26},{94,-26},{94,-2}},       color={0,127,255}));
  for k in 1:n - 1 loop
    connect(
  layer_HE[k].ports[2], layer_HE[k + 1].ports[1]);
  end for;
  //Connect heat exchanger to storage layers
  for k in 1:n loop
    connect(
  heatTransfer_HE[k].port_a, layer[k].heatPort);
    connect(
  heatTransfer_HE[k].port_b, layer_HE[k].heatPort);
  end for;
  //Connect bouyancy element
  for k in 1:n - 1 loop
    connect(
  bouyancy[k].port_a, layer[k + 1].heatPort);
    connect(
  bouyancy[k].port_b, layer[k].heatPort);
  end for;
  connect(
heatPort, heatPort)
    annotation (Line(points={{-106,0},{-106,0}}, color={191,0,0}));

  connect(port_b_consumer_bot, layer[1].ports[2]) annotation (Line(points={{-40,92},
          {-40,24},{16,24},{16,2},{10,2}},             color={0,127,255}));
  connect(port_a_consumer_cooling, layer[n].ports[1])
    annotation (Line(points={{10,-98},{10,-2}},      color={0,127,255}));
  connect(port_a_heatGenerator_cooling, layer_HE[1].ports[2])
    annotation (Line(points={{94,92},{94,2}},            color={0,127,255}));
  connect(port_b_heatGenerator_cooling, layer_HE[n].ports[1]) annotation (Line(
