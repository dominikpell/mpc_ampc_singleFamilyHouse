...
model ThermalZone "Thermal zone containing moisture balance"
  extends
    MA_Pell_SingleFamilyHouse.Systems.Subsystems.Demand.BaseClasses.PartialThermalZone(
      ROM(
      T_Air=T_Air,
      T_Roof=T_Roof,
      T_IntWall=T_IntWall,
      ...
      T_Floor=T_Floor,
      T_Win=T_Win));
  parameter Real T_Roof=T_Roof annotation(Dialog(tab="Initialization"));
  parameter Real T_Air=T_Air  annotation(Dialog(tab="Initialization"));
  ...
  parameter Real T_Floor=T_Floor
                                annotation(Dialog(tab="Initialization"));
  parameter Real T_Win=T_Win  annotation(Dialog(tab="Initialization"));
  ...
  AixLib.ThermalZones.ReducedOrder.EquivalentAirTemperature.VDI6007WithWindow eqAirTempWall(
    ...
    final TGro=zoneParam.TSoil) if (sum(zoneParam.AExt) + sum(zoneParam.AWin)) >
    0 "Computes equivalent air temperature"
  ...
  AixLib.BoundaryConditions.SolarIrradiation.DirectTiltedSurface HDirTilWall[zoneParam.nOrientations](
    each final lat=zoneParam.lat,
    final azi=zoneParam.aziExtWalls,
    final til=zoneParam.tiltExtWalls)
    "Calculates direct solar radiation on titled surface for both directions"
    annotation (Placement(transformation(extent={{-84,31},{-68,48}})));
  AixLib.BoundaryConditions.SolarIrradiation.DirectTiltedSurface HDirTilRoof[zoneParam.nOrientationsRoof](
    each final lat=zoneParam.lat,
  ...
  AixLib.Utilities.Sources.HeaterCooler.HeaterCoolerPI heaterCooler(
    ...
    not recOrSep and (Heater_on or Cooler_on)) "Heater Cooler with PI control"
  ...
  AixLib.ThermalZones.ReducedOrder.SolarGain.SimpleExternalShading simpleExternalShading(
    final nOrientations=zoneParam.nOrientations,
    final maxIrrs=zoneParam.maxIrr,
    final gValues=zoneParam.shadingFactor) if sum(zoneParam.ATransparent) > 0
    annotation (Placement(transformation(extent={{4,44},{10,50}})));

  // Air Exchange
  AixLib.Controls.VentilationController.VentilationController ventCont(
    ...
    final Tmean_start=zoneParam.T_start) if (ATot > 0 or zoneParam.VAir > 0)
     and use_AirExchange "Calculates natural venitlation and infiltration"
    annotation (Placement(transformation(extent={{-68,-34},{-48,-14}})));
  AixLib.Utilities.Psychrometrics.MixedTemperature mixedTemp if (ATot > 0 or
    zoneParam.VAir > 0) and use_AirExchange
  ...
  AixLib.ThermalZones.HighOrder.Components.DryAir.VarAirExchange airExc(final V=
        zoneParam.VAir) if (ATot > 0 or zoneParam.VAir > 0) and use_AirExchange
     and not use_moisture_balance "Heat flow due to ventilation"
  ...
  Modelica.Blocks.Math.MultiSum SumQLat2_flow(nu=3) if (ATot > 0 or
    ...
    annotation (Placement(transformation(extent={{-40,-68},{-28,-56}})));
  AixLib.BoundaryConditions.InternalGains.Moisture.MoistureGains moistureGains(final
      roomArea=zoneParam.AZone, final specificMoistureProduction=zoneParam.internalGainsMoistureNoPeople) if
       ATot > 0 and use_moisture_balance
    "Internal moisture gains by plants, etc." annotation (Dialog(enable=
          use_moisture_balance, tab="Moisture"), Placement(transformation(
          extent={{-70,-78},{-60,-68}})));
  ...
  Modelica.Blocks.Interfaces.RealInput ventHum(
    ...
    "Ventilation and infiltration humidity" annotation (Placement(
        transformation(extent={{-128,-108},{-88,-68}}), iconTransformation(
          extent={{-106,-82},{-84,-60}})));
  AixLib.ThermalZones.HighOrder.Components.MoistAir.VarMoistAirExchange airExcMoi(final V=
        zoneParam.VAir) if (ATot > 0 or zoneParam.VAir > 0) and use_AirExchange
     and use_moisture_balance "Heat flow due to ventilation"
    annotation (Placement(transformation(extent={{-22,-12},{-6,4}})));

  // CO2
  AixLib.BoundaryConditions.InternalGains.CO2.CO2Balance cO2Balance(
...
equation
  ...
  connect(TSetCool, heaterCooler.setPointCool) annotation (Line(points={{-108,8},
          {102,8},{102,14},{70.36,14},{70.36,28.8}},
                                                   color={0,0,127}));
