...
model OneElement "Thermal Zone with one element for exterior walls"
  ...
  AixLib.ThermalZones.ReducedOrder.RC.BaseClasses.ThermSplitter thermSplitterIntGains(
    final splitFactor=splitFactor,
    final nOut=dimension,
    final nIn=1) if ATot > 0 "Splits incoming internal gains into separate gains for each wall element,
    weighted by their area"
    annotation (Placement(transformation(extent={{210,76},{190,96}})));
  AixLib.ThermalZones.ReducedOrder.RC.BaseClasses.ThermSplitter thermSplitterSolRad(
    final splitFactor=splitFactorSolRad,
    final nOut=dimension,
    final nIn=nOrientations) if ATot > 0 and sum(ATransparent) > 0 "Splits incoming solar radiation into separate gains for each wall element,
    weighted by their area"
  ...
  AixLib.ThermalZones.ReducedOrder.RC.BaseClasses.ExteriorWall extWallRC(
    ...
    final T_start=T_ExtWall) if ATotExt > 0 "RC-element for exterior walls"
...
equation
  ...
  if ATotExt > 0 and ATotWin > 0 then
    ...
    connect(thermSplitterIntGains.portOut[2], convWin.solid)
      annotation (
      Line(points={{190,86},{190,84},{-120,84},{-120,40},{-116,40}},
      color={191,0,0}));
