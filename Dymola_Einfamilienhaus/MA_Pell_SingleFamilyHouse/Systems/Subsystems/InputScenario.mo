...
package InputScenario

  model Scenario
    "Standard Scenario"
    extends Systems.Subsystems.InputScenario.BaseClasses.PartialInputScenario;
    MA_Pell_SingleFamilyHouse.Systems.Subsystems.InputScenario.ReaderTMY3 weaDat(
      ...
      "Weather data reader"
      annotation (Placement(transformation(extent={{-74,192},{0,260}})));

    Modelica.Blocks.Sources.Constant tableInternalGains1(k=systemParameters.TSoilConst)
    ...
    Modelica.Blocks.Interfaces.RealInput TDryBulb
      annotation (Placement(transformation(extent={{-220,224},{-180,264}})));
    Modelica.Blocks.Interfaces.RealInput ElecDemand
      annotation (Placement(transformation(extent={{-220,40},{-180,80}}),
          iconTransformation(extent={{-220,40},{-180,80}})));
    Modelica.Blocks.Interfaces.RealInput EVDemand
      annotation (Placement(transformation(extent={{-220,-32},{-180,8}}),
          iconTransformation(extent={{-220,-32},{-180,8}})));
    Modelica.Blocks.Interfaces.RealInput H_GloHor
      annotation (Placement(transformation(extent={{-220,108},{-180,148}}),
          iconTransformation(extent={{-220,108},{-180,148}})));
    Modelica.Blocks.Interfaces.RealInput IntGains[3]
      annotation (Placement(transformation(extent={{-218,-236},{-178,-196}}),
          iconTransformation(extent={{-218,-236},{-178,-196}})));
    Modelica.Blocks.Interfaces.RealInput mFlow_DHW
      annotation (Placement(transformation(extent={{-218,-108},{-178,-68}}),
          iconTransformation(extent={{-218,-108},{-178,-68}})));
    Modelica.Blocks.Interfaces.RealInput TDem_DHW
      annotation (Placement(transformation(extent={{-218,-172},{-178,-132}}),
          iconTransformation(extent={{-218,-172},{-178,-132}})));
    Modelica.Blocks.Interfaces.RealInput winSpe
      annotation (Placement(transformation(extent={{-220,166},{-180,206}}),
          iconTransformation(extent={{-220,166},{-180,206}})));
  equation
    connect(weaDat.weaBus, inputScenBus.weaBus) annotation (Line(
        points={{0.925,226.17},{138,226.17},{138,182},{203.175,182},{203.175,
            -0.835}},
        color={255,204,51},
        thickness=0.5), Text(
        string="%second",
    ...
    connect(tableInternalGains1.y, inputScenBus.TSoil) annotation (Line(points={{1.8,
            -198},{202,-198},{202,-98},{203.175,-98},{203.175,-0.835}},
                                                    color={0,0,127}), Text(
        string="%second",
        index=1,
        extent={{6,3},{6,3}},
        horizontalAlignment=TextAlignment.Left));
    connect(weaDat.weaBus, inputScenBus.electricityBus.PVBus.waeBus)
      annotation (Line(
        points={{0.925,226.17},{203,226.17},{203,-1}},
        color={255,204,51},
        thickness=0.5));
    connect(TDryBulb, weaDat.TDryBul_in) annotation (Line(points={{-200,244},{
            -136,244},{-136,256.6},{-77.7,256.6}},
                                              color={0,0,127}));
    connect(ElecDemand, inputScenBus.ElectricityDemand) annotation (Line(points={{-200,60},
            {203.175,60},{203.175,-0.835}},            color={0,0,127}));
    connect(EVDemand, inputScenBus.EVDemand) annotation (Line(points={{-200,-12},
            {203.175,-12},{203.175,-0.835}},color={0,0,127}));
    connect(IntGains, inputScenBus.intGains) annotation (Line(points={{-198,
            -216},{203.175,-216},{203.175,-0.835}},      color={0,0,127}), Text(
        string="%second",
        index=1,
        extent={{6,3},{6,3}},
        horizontalAlignment=TextAlignment.Left));
    connect(mFlow_DHW, inputScenBus.m_flowDHW) annotation (Line(points={{-198,
            -88},{203.175,-88},{203.175,-0.835}},
                                            color={0,0,127}), Text(
        string="%second",
        index=1,
        extent={{6,3},{6,3}},
        horizontalAlignment=TextAlignment.Left));
    connect(TDem_DHW, inputScenBus.TDemandDHW) annotation (Line(points={{-198,
            -152},{-124,-152},{-124,-14},{200,-14},{200,-0.835},{203.175,-0.835}},
                                                                            color=
           {0,0,127}), Text(
        string="%second",
        index=1,
        extent={{6,3},{6,3}},
        horizontalAlignment=TextAlignment.Left));
    connect(H_GloHor, weaDat.HGloHor_only) annotation (Line(points={{-200,128},
            {-140,128},{-140,169.22},{-77.7,169.22}}, color={0,0,127}));
    connect(winSpe, weaDat.winSpe_in) annotation (Line(points={{-200,186},{-158,
            186},{-158,212.74},{-77.7,212.74}}, color={0,0,127}));
  ...
  model HeatDemandScenario
    "Scenario for heat demand calculation"
    extends Systems.Subsystems.InputScenario.BaseClasses.PartialInputScenario;

    Modelica.Blocks.Sources.Constant const2(k=0)
      annotation (Placement(transformation(extent={{-218,-198},{-146,-126}})));

    Modelica.Blocks.Sources.Constant
                                 T_set_const1(k=T_const)
      annotation (Placement(transformation(extent={{-218,4},{-140,82}})));
    ...
    AixLib.BoundaryConditions.WeatherData.ReaderTMY3 weaDat(
      ...
      filNam=Modelica.Utilities.Files.loadResource(
          "modelica://BuildingEnergySystems/Resources/TRY2015_522361130393_Jahr_City_Potsdam.mos"))
      "Weather data reader"
      annotation (Placement(transformation(extent={{-88,-54},{0,52}})));

    Modelica.Blocks.Sources.Constant const1(k=0)
      annotation (Placement(transformation(extent={{-298,-100},{-226,-28}})));
    Modelica.Blocks.Sources.Constant const3(k=0)
      annotation (Placement(transformation(extent={{-36,-258},{36,-186}})));
    Modelica.Blocks.Sources.Constant const4(k=0)
      annotation (Placement(transformation(extent={{230,-242},{302,-170}})));
    Modelica.Blocks.Sources.Constant const5(k=0)
      annotation (Placement(transformation(extent={{46,-150},{118,-78}})));
    Modelica.Blocks.Sources.Constant
                                 T_set_const2(k=293.15)
      annotation (Placement(transformation(extent={{-118,118},{-40,196}})));
    Modelica.Blocks.Sources.Constant const6(k=systemParameters.TSoilConst)
      annotation (Placement(transformation(extent={{46,38},{118,110}})));
  equation
    connect(const2.y, inputScenBus.m_flowDHW) annotation (Line(points={{-142.4,
            -162},{203.175,-162},{203.175,-0.835}}, color={0,0,127}), Text(
        string="%second",
        index=1,
        extent={{6,3},{6,3}},
        horizontalAlignment=TextAlignment.Left));
    connect(inputScenBus.weaBus, weaDat.weaBus) annotation (Line(
        points={{203.175,-0.835},{98,-0.835},{98,-1},{0,-1}},
        color={255,204,51},
        thickness=0.5), Text(
        string="%first",
        index=-1,
    ...
    connect(T_set_const1.y, weaDat.TDryBul_in) annotation (Line(points={{-136.1,
            43},{-92.4,43},{-92.4,46.7}},   color={0,0,127}));
    connect(const3.y, inputScenBus.intGains[1]) annotation (Line(points={{39.6,
            -222},{203.175,-222},{203.175,-0.835}}, color={0,0,127}), Text(
        string="%second",
        index=1,
        extent={{6,3},{6,3}},
        horizontalAlignment=TextAlignment.Left));
    connect(const4.y, inputScenBus.intGains[2]) annotation (Line(points={{305.6,
            -206},{305.6,-101},{203.175,-101},{203.175,-0.835}}, color={0,0,127}),
        Text(
        string="%second",
        index=1,
        extent={{-3,6},{-3,6}},
        horizontalAlignment=TextAlignment.Right));
    connect(const5.y, inputScenBus.intGains[3]) annotation (Line(points={{121.6,
            -114},{174,-114},{174,-0.835},{203.175,-0.835}}, color={0,0,127}),
        Text(
        string="%second",
        index=1,
        extent={{6,3},{6,3}},
        horizontalAlignment=TextAlignment.Left));
    connect(const1.y, weaDat.HGloHor_in) annotation (Line(points={{-222.4,-64},{
            -174.2,-64},{-174.2,-69.9},{-92.4,-69.9}},  color={0,0,127}));
    connect(const1.y, weaDat.HDifHor_in) annotation (Line(points={{-222.4,-64},{-176.2,
            -64},{-176.2,-51.35},{-92.4,-51.35}},         color={0,0,127}));
    connect(T_set_const2.y, inputScenBus.TDemandDHW) annotation (Line(points={{
            -36.1,157},{203.175,157},{203.175,-0.835}}, color={0,0,127}), Text(
        string="%second",
        index=1,
        extent={{6,3},{6,3}},
        horizontalAlignment=TextAlignment.Left));
    connect(const6.y, inputScenBus.TSoil) annotation (Line(points={{121.6,74},{
            203.175,74},{203.175,-0.835}}, color={0,0,127}), Text(
        string="%second",
        index=1,
        extent={{6,3},{6,3}},
        horizontalAlignment=TextAlignment.Left));
  end HeatDemandScenario;

  package BaseClasses

    partial model PartialInputScenario
      "Partial model for replaceable scenarios"
      replaceable parameter
        ...
        "Parameters relevant for the whole energy system" annotation (
          choicesAllMatching=true, Placement(transformation(extent={{162,-256},
                {194,-220}})));
      Interfaces.InputScenarioBus inputScenBus
        annotation (Placement(transformation(extent={{168,-34},{238,32}})));

      annotation (Icon(coordinateSystem(preserveAspectRatio=false, extent={{-200,-260},
                {200,260}}), graphics={
            Rectangle(
              extent={{-200,260},{200,4}},
              lineColor={124,142,255},
              fillColor={124,142,255},
              fillPattern=FillPattern.Solid),
            Polygon(
              points={{110,234},{93.973,170.984},{94,170},{126,180},{154,178},{180,166},
                  {198,100},{154,26},{26,18},{-124,22},{-150,98},{-134,152},{-86,154},
                  {-62.2109,136.158},{-62,136},{-76,198},{-42,248},{50,268},{110,234}},
            ...
            Ellipse(
              extent={{-198,270},{-24,96}},
              lineColor={255,220,220},
              lineThickness=1,
              fillPattern=FillPattern.Sphere,
              fillColor={255,255,0}),
            Rectangle(
              extent={{-200,6},{202,-260}},
              lineColor={124,142,255},
              fillColor={138,140,132},
              fillPattern=FillPattern.Solid),
            Text(
              extent={{-194,-180},{192,-292}},
              lineColor={0,0,0},
              textString="%name%
")}),     Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-200,-260},{200,
                260}})));
    end PartialInputScenario;
  ...
  block ReaderTMY3 "Reader for TMY3 weather data"
    ...
    AixLib.BoundaryConditions.WeatherData.Bus weaBus "Weather data bus"
      annotation (Placement(transformation(extent={{290,-10},{310,10}}),
          iconTransformation(extent={{174,-28},{236,30}})));
    ...
    parameter AixLib.BoundaryConditions.Types.DataSource TDryBulSou=AixLib.BoundaryConditions.Types.DataSource.File
      "Dry bulb temperature"
      annotation (Evaluate=true, HideResult=true, Dialog(tab="Data source"));
    parameter Modelica.SIunits.Temperature TDryBul=293.15
    ...
    parameter AixLib.BoundaryConditions.Types.DataSource TDewPoiSou=AixLib.BoundaryConditions.Types.DataSource.File
      "Dew point temperature"
      annotation (Evaluate=true, HideResult=true, Dialog(tab="Data source"));
    parameter Modelica.SIunits.Temperature TDewPoi=283.15
    ...
    parameter Modelica.SIunits.Angle winDir=1
    ...
    Modelica.Blocks.Interfaces.RealInput HInfHor_in(
     ...
     parameter AixLib.BoundaryConditions.Types.RadiationDataSource HSou=AixLib.BoundaryConditions.Types.RadiationDataSource.File
      "Global, diffuse, and direct normal radiation"
       annotation (Evaluate=true, HideResult=true, Dialog(tab="Data source"));
     parameter Boolean useHGloHorInput_only = true "gets Global Horizontal Radiation from a single Input"
       annotation (Evaluate=true, HideResult=true, Dialog(tab="Data source"));
    //--------------------------------------------------------------
  ...
  //--------------------------------------------------------------
    ...
    Modelica.Blocks.Interfaces.RealInput HGloHor_only if useHGloHorInput_only==true
      annotation (Placement(transformation(extent={{-240,-354},{-200,-314}})));
    Modelica.Blocks.Logical.Switch switch1
      annotation (Placement(transformation(extent={{244,110},{264,130}})));
    Modelica.Blocks.Sources.BooleanExpression booleanExpression(y=
          useHGloHorInput_only)
      annotation (Placement(transformation(extent={{152,110},{172,130}})));
  protected
    ...
    AixLib.BoundaryConditions.WeatherData.BaseClasses.SourceSelector pAtmSel(final
        datSou=pAtmSou, final p=pAtm) "Source selection for atmospheric pressure"
      annotation (Placement(transformation(extent={{0,260},{20,280}})));
    AixLib.BoundaryConditions.WeatherData.BaseClasses.SourceSelector TDewPoiSel(final
        datSou=TDewPoiSou, final p=TDewPoi)
      "Source selection for dewpoint temperature pressure"
      annotation (Placement(transformation(extent={{92,-240},{112,-220}})));
    AixLib.BoundaryConditions.WeatherData.BaseClasses.SourceSelector TDryBulSel(final
        datSou=TDryBulSou, final p=TDryBul)
      "Source selection for drybulb temperature pressure"
      annotation (Placement(transformation(extent={{92,-200},{112,-180}})));
    AixLib.BoundaryConditions.WeatherData.BaseClasses.SourceSelector TBlaSkySel(final
        datSou=TBlaSkySou, final p=TBlaSky)
      "Source selection for sky black body radiation"
      annotation (Placement(transformation(extent={{240,-180},{260,-160}})));
    AixLib.BoundaryConditions.WeatherData.BaseClasses.SourceSelector relHumSel(final
        datSou=relHumSou, final p=relHum)
      "Source selection for relative humidity"
      annotation (Placement(transformation(extent={{120,20},{140,40}})));
    AixLib.BoundaryConditions.WeatherData.BaseClasses.SourceSelector opaSkyCovSel(final
        datSou=opaSkyCovSou, final p=opaSkyCov)
      "Source selection for opaque sky cover"
      annotation (Placement(transformation(extent={{120,-160},{140,-140}})));
    AixLib.BoundaryConditions.WeatherData.BaseClasses.SourceSelector ceiHeiSel(final
        datSou=ceiHeiSou, final p=ceiHei) "Source selection for ceiling height"
      annotation (Placement(transformation(extent={{120,-120},{140,-100}})));
    AixLib.BoundaryConditions.WeatherData.BaseClasses.SourceSelector totSkyCovSel(final
        datSou=totSkyCovSou, final p=totSkyCov)
      "Source selection for total sky cover"
      annotation (Placement(transformation(extent={{120,-40},{140,-20}})));
    AixLib.BoundaryConditions.WeatherData.BaseClasses.SourceSelector winSpeSel(final
        datSou=winSpeSou, final p=winSpe) "Source selection for wind speed"
      annotation (Placement(transformation(extent={{120,-80},{140,-60}})));
    AixLib.BoundaryConditions.WeatherData.BaseClasses.SourceSelector winDirSel(final
        datSou=winDirSou, final p=winDir) "Source selection for wind speed"
      annotation (Placement(transformation(extent={{120,-280},{140,-260}})));
    AixLib.BoundaryConditions.WeatherData.BaseClasses.SourceSelector horInfRadSel(final
        datSou=HInfHorSou, final p=HInfHor)
      "Source selection for horizontal infrared radiation"
      annotation (Placement(transformation(extent={{120,60},{140,80}})));
    AixLib.BoundaryConditions.WeatherData.BaseClasses.SourceSelectorRadiation souSelRad(final
        datSou=HSou) "Source selection for solar irradiation"
      annotation (Placement(transformation(extent={{120,180},{140,200}})));

    AixLib.BoundaryConditions.WeatherData.BaseClasses.CheckDryBulbTemperature cheTemDryBul
    ...
    AixLib.BoundaryConditions.SkyTemperature.BlackBody TBlaSkyCom(final calTSky=
          calTSky) if TBlaSkySou == AixLib.BoundaryConditions.Types.DataSource.File
    ...
    AixLib.BoundaryConditions.SolarGeometry.BaseClasses.AltitudeAngle altAng
     ...
     AixLib.BoundaryConditions.SolarGeometry.BaseClasses.ZenithAngle zenAng(final lat=
         lat) "Zenith angle"
...
First implementation.
  ...
  equation
    ...
    connect(souSelRad.HGloHor, switch1.u3) annotation (Line(points={{141,182},{
            198,182},{198,112},{242,112}}, color={0,0,127}));
    connect(switch1.y, weaBus.HGloHor) annotation (Line(points={{265,120},{284,
            120},{284,0},{300,0}}, color={0,0,127}));
    connect(HGloHor_only, switch1.u1) annotation (Line(points={{-220,-334},{320,
            -334},{320,136},{234,136},{234,128},{242,128}}, color={0,0,127}));
    connect(booleanExpression.y, switch1.u2)
      annotation (Line(points={{173,120},{242,120}}, color={255,0,255}));
      annotation (
