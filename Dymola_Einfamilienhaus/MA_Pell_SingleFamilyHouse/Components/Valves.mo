...
package Valves
  model ArtificialThreeWayValve
    ...
    Pumps.ArtificalPump_h_in artificalPump_h_in_gen(redeclare package Medium =
          Medium)                                   annotation (Placement(
          transformation(
          extent={{-10,-10},{10,10}},
          rotation=270,
          origin={-78,0})));
    Pumps.ArtificalPump_h_in artificalPump_h_in_buf(redeclare package Medium =
          Medium, p=p_hydr)                         annotation (Placement(
          transformation(
          extent={{-10,-10},{10,10}},
          rotation=90,
          origin={68,-60})));
    Pumps.ArtificalPump_h_in artificalPump_h_in_dhw(redeclare final package
        Medium = Medium, final p=p_hydr)            annotation (Placement(
          transformation(
          extent={{-10,-10},{10,10}},
          rotation=90,
          origin={66,60})));
  ...
  package Tests
    ...
    model Test_ArtificalThreeWayValve
      ...
      ArtificialThreeWayValve artificialThreeWayValve(redeclare package
          Medium =
            Medium)
      ...
      Modelica.Fluid.Sources.Boundary_ph boundary2(redeclare package Medium =
        ...
        annotation (Placement(transformation(extent={{10,-10},{-10,10}},
            rotation=180,
            origin={-74,-12})));
      AixLib.Fluid.MixingVolumes.MixingVolume demandDHW(
        each V=0.005,
        redeclare package Medium = Medium,
        each nPorts=2,
        each m_flow_nominal=0.01) annotation (Placement(transformation(
            ...
            rotation=270,
            origin={52,-14})));
      AixLib.Fluid.MixingVolumes.MixingVolume demandBui(
        each V=0.005,
        redeclare package Medium = Medium,
        each nPorts=2,
        each m_flow_nominal=0.01) annotation (Placement(transformation(
