...
extends Modelica.Icons.BasesPackage;
  ...
  model calcBat "calculates Loads and Charging Power Grid"
    Modelica.Blocks.Interfaces.RealInput PowerBatUse annotation (Placement(
          transformation(
          extent={{-20,-20},{20,20}},
          rotation=90,
          origin={-40,-100})));
    Modelica.Blocks.Interfaces.RealInput PowerFeedIn annotation (Placement(
          transformation(
          extent={{-20,-20},{20,20}},
          rotation=90,
          origin={40,-100})));
    Modelica.Blocks.Interfaces.RealInput PowerChargeTotal annotation (Placement(
          transformation(
          extent={{-20,-20},{20,20}},
          rotation=90,
          origin={0,-100})));
    Modelica.Blocks.Interfaces.RealInput PowerChargePV
      annotation (Placement(transformation(extent={{-120,-20},{-80,20}})));
    Real PCharge
      annotation ();
    Real PLoad
      annotation ();
    Modelica.Blocks.Interfaces.RealOutput PFeedIn
      annotation (Placement(transformation(extent={{90,-50},{110,-30}})));
    Modelica.Blocks.Interfaces.RealOutput PUse
      annotation (Placement(transformation(extent={{90,30},{110,50}})));
  equation
    //PCharge = if PowerChargeTotal - PowerChargePV < 0 then 0 else PowerChargeTotal - PowerChargePV;
    PCharge = PowerChargeTotal - PowerChargePV;
    PLoad = PowerBatUse + PowerFeedIn;
    connect(PowerFeedIn, PFeedIn)
      annotation (Line(points={{40,-100},{40,-40},{100,-40}}, color={0,0,127}));
    connect(PowerBatUse, PUse) annotation (Line(points={{-40,-100},{-40,40},{
